...
it(`generates ${ GENERATOR_NAME.toUpperCase() } file using defaults`,
  async () => {
    // GIVEN
    let opts = {
...
it(`generates ${ GENERATOR_NAME.toUpperCase() } file with options`,
  async () => {
    // GIVEN
    let opts = {
      generator: GENERATOR_NAME,
  ...
  async () => {
    ...
    let opts = {
      ...
      content: [{
        ...
        regex: /class\s*=\s*["'|][^"'|]+["'|]/g,
        mapper: className => {
          return (className.match(/class\s*=\s*["'|]([^"'|]+)["'|]/)[1])
            .match(/\S+/g)
        },
        escape: true
      }]
    ...
    let expectedCss = fs.readFileSync(
      path.resolve(__dirname, '../expected_data/purge_expected_data_ext.css'), 'utf8'
    )
...
it(`generates ${ GENERATOR_NAME.toUpperCase() } file with string path`,
  async () => {
    // GIVEN
    let opts = {
      generator: GENERATOR_NAME,
...
it(`generates purged ${ GENERATOR_NAME.toUpperCase() } file`,
  async () => {
    // GIVEN
    let opts = {
      generator: GENERATOR_NAME,
